...
/* ritorna una risposta se non ha bisogno di info aggiuntive, altrimenti ritorna il nome di quelle che gli servono */
...
/* risponde a domande con piccole quantità di dati allegati */
...
/* risponde a domande con grandi quantità di dati allegati */
module.exports.askConversionFile = async (user_request, history, output) => {
    if (!data || !data.text) {
        throw Error("No data provided for completion");
    }
    const prompt = `${user_request} ${data.label} disponibili: ${data.text}`;
    ...
    const command = new ConverseCommand({
        modelId: "amazon.nova-lite-v1:0",
        messages: processHistory(prompt, history, output),
        system: [{ content: assistant_context }]
    });
    const response = await client.send(command);
...
const processHistory = (user_request, history, output={}) => {
    messages = [];
